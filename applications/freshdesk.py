...
# Ticket Actions
async def freshdesk_get_ticket(json_cred, params, **kwargs):
    """
    ...
    try:
        ...
        if "Expecting value" in str(err):
            return {"Error": "Invalid Domain OR Id Not Found"}
        else:
            return {"Error": str(err)}

async def freshdesk_get_all_tickets(json_cred, params, **kwargs):
    """
...
async def freshdesk_create_ticket(json_cred, params, **kwargs):
    """
    ...
    try:
        ...
        if "apiKey" in creds and "domain" in creds and all(item in params for item in allRequired) and any(item in params for item in oneRequired):
            ...
            if ".freshdesk.com" not in domain:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, auth=BasicAuth(apiKey, "X"), json=data) as response:
                        response.raise_for_status()
                        result = await response.json()
                for item in result:
                    if item == "errors" or item == "code":
...
async def freshdesk_delete_ticket(json_cred, params, **kwargs):
    """
    ...
    try:
        ...
        if "apiKey" in creds and "domain" in creds and "ticket_id" in params:
            ...
            if ".freshdesk.com" not in domain:
                async with aiohttp.ClientSession() as session:
                    async with session.delete(url, auth=BasicAuth(apiKey, "X")) as response:
                        response.raise_for_status()
                        if response.status in successStatus:
                            return {"Result": f"Deleted ticket ID: {ticket_id}"}
                        raise Exception(
                            f"Failed to delete the ticket. Status Code: {response.status}. Response: {await response.text()}"
                    )
            else:
                raise Exception(f"Invalid URL detected. Please verify your domain name. Domain: {domain}, URL: {url}")
        else:
...
async def freshdesk_update_ticket(json_cred, params, **kwargs):
    """
            ...
            if ".freshdesk.com" not in domain:
                ...
                for key, value in params.items():
                    ...
                    if value:
                        data["ticket"][key] = value
                async with aiohttp.ClientSession() as session:
                    async with session.put(url, auth=BasicAuth(apiKey, "X"), json=data) as response:
                        response.raise_for_status()
                        result = await response.json()
                for item in result:
...
async def freshdesk_get_contact(json_cred, params, **kwargs):
    """
...
async def freshdesk_get_all_contacts(json_cred, params, **kwargs):
    """
...
async def freshdesk_create_contact(json_cred, params, **kwargs):
    """
...
async def freshdesk_delete_contact(json_cred, params, **kwargs):
    """
            ...
            if ".freshdesk.com" not in domain:
                async with aiohttp.ClientSession() as session:
                    async with session.delete(url, auth=BasicAuth(apiKey, "X")) as response:
                        ...
                        raise Exception(
                            f"Failed to delete the contact. Status Code: {response.status}. Response: {await response.text()}"
                    )
            else:
                raise Exception(f"Invalid URL detected. Please verify your domain name. Domain: {domain}, URL: {url}")
        else:
...
async def freshdesk_update_contact(json_cred, params, **kwargs):
    """
...
operations = {
    'CREATE TICKET':freshdesk_create_ticket,
    'GET MANY TICKETS':freshdesk_get_all_tickets,
    'DELETE TICKET':freshdesk_delete_ticket,
    'UPDATE TICKET':freshdesk_update_ticket,
    'GET TICKET':freshdesk_get_ticket,
    'CREATE CONTACT':freshdesk_create_contact,
    'GET CONTACT':freshdesk_get_contact,
    'GET MANY CONTACTS':freshdesk_get_all_contacts,
    'UPDATE CONTACT':freshdesk_update_contact,
    'DELETE CONTACT':freshdesk_delete_contact
}