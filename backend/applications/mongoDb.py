...
def mongodb_find_documents(cred,params,**kwargs):
    """
    ...
    try:
        ...
        if "uri" in creds and "databaseName" in creds and "collectionName" in params:
            ...
            if sort:
                documents = collection.find(query).limit(limit).sort(sort).to_list()
            else:
                documents = collection.find(query).limit(limit).to_list()
            try:
                result = json.loads(json.dumps(documents))
...
def mongodb_delete_many_document(cred,params,**kwargs):
    """
...
def mongodb_insert_one_document(cred,params,**kwargs):
    """
...
def mongodb_insert_many_document(cred,params,**kwargs):
    """
...
def mongodb_update_documents(cred,params,**kwargs):
    """
...
def mongodb_find_one_and_update_document(cred,params,**kwargs):
    """
...
def mongodb_find_one_and_replace_document(cred,params,**kwargs):
    """
...
def mongodb_aggregate_documents(cred,params,**kwargs):
    """
    ...
    try:
        ...
        if "uri" in creds and "databaseName" in creds and "collectionName" in params and "matchData" in params and "groupData" in params:
            ...
            try:
                result = json.loads(json.dumps(results))
            except Exception as e:
...
operations = {
    'Find Documents': mongodb_find_documents,
    'Delete Many Document': mongodb_delete_many_document,
    'Insert Document': mongodb_insert_one_document,
    'Insert Many Document': mongodb_insert_many_document,
    'Update Documents': mongodb_update_documents, 
    'Find One And Update Document': mongodb_find_one_and_update_document, 
    'Find One And Replace Document': mongodb_find_one_and_replace_document,
    'Aggregate Documents': mongodb_aggregate_documents,
}
