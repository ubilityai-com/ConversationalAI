...
def hubspot_delete_contact(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            return {"message":f"contact of id {params['id']} successfully deleted"}
    except Exception as e:
        raise Exception(e)

def hubspot_search_contact(access_token, params):
    """
    ...
    :param dict params: filters the properties to be returned for the retrieved contact.
        ...
        :query: (str,optional) Search query string to filter contacts by a keyword.
        :filterGroups: (list,optional): A list of filter group dictionaries. Each dictionary contains "filters", 

            which is a list of filter conditions where each filter has "propertyName", "operator", and "value".

        :sorts: (list,optional): A list of sort conditions where each dictionary has "propertyName" and "direction".

            Example: [{"propertyName": "firstname", "direction": "ASCENDING"}].

    :return: JSON response details about the contact
    :rtype: dict  
    """
    try:
        ...
        contact = requests.post(url=url, headers=headers, json=data)
        if contact.status_code == 404:
            raise Exception("invalid input data")
        else:
            return contact.json()
...
def hubspot_delete_company(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            requests.delete(url, headers=headers)
            return {"message":f"company of id {params['id']} successfully deleted"}
        else:
            raise Exception("missing input data")
    except Exception as e:
        raise Exception(e)
...
def hubspot_update_ticket(access_token, params):
    ...
    try:
        if 'id' in params:
            ticket = {}
            keys_to_skip = ['ticketCreateTime', 'ticketCreateDate',
                            'ticketCloseTime', 'ticketCloseDate', "id"]
            for key, value in params.items():
                if key not in keys_to_skip:
                    ticket[key] = value
            headers = {
...
def hubspot_delete_ticket(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            requests.delete(url, headers=headers)
            return {"message":f"ticket of id {params['id']} successfully deleted"}
        else:
            raise Exception("Missing ticket ID")
    except Exception as e:
        raise Exception(e)
...
def hubspot_update_deal(access_token, params):
    ...
    try:
        if 'id' in params:
            deal = {}
            keys_to_skip = ['id', 'dealCreateTime',
                            'dealCreateDate', 'dealCloseTime', 'dealCloseDate']
            for key, value in params.items():
...
def hubspot_delete_deal(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            requests.delete(url, headers=headers)
            return {"message":f"deal of id {params['id']} successfully deleted"}
        else:
            raise Exception("missing input data")
    except Exception as e:
        raise Exception(e)
...
def hubspot_get_many_calls(access_token, params):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Collect all 'CALL' type engagements
            for engagement in engagements.get('results', []):
                if engagement['engagement']['type'] == 'CALL':
                    call_engagements.append(engagement)
            # If an association_id is provided in params, filter engagements based on it
            if params and "association_id" in params:
                association_id = int(params["association_id"])  # Ensure association_id is an integer
                for engagement in call_engagements:
                    associations = engagement.get('associations', {})
                    # Check all possible association lists for the association_id
                    for ids in associations.values():
                        if association_id in ids:
                            filtered_engagements.append(engagement)
                return filtered_engagements[:limit] if limit else filtered_engagements
            # If no association_id is provided, return all call engagements
            return call_engagements[:limit] if limit else call_engagements
        else:
...
def hubspot_delete_call(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            requests.delete(url, headers=headers)
            return {"message":f"call of id {params['id']} successfully deleted"}
        else:
            raise Exception("missing input data")
    except Exception as e:
        raise Exception(e)
...
def hubspot_delete_email(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            requests.delete(url, headers=headers)
            return {"message":f"email of id {params['id']} successfully deleted"}
        else:
            raise Exception("missing input data")
    except Exception as e:
        raise Exception(e)
...
def hubspot_delete_task(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            requests.delete(url, headers=headers)
            return {"message":f"task of id {params['id']} successfully deleted"}
        else:
            raise Exception("missing input data")
    except Exception as e:
        raise Exception(e)
...
def hubspot_create_meeting(access_token, params):
    ...
    try:
        if 'hs_timestamp' in params['properties']:
            meeting = {}
            keys_to_skip = ['meetingStartTime', 'meetingStartDate',
                            'meetingEndTime', 'meetingEndDate']
            for key, value in params.items():
                if key not in keys_to_skip:
                    meeting[key] = value
            headers = {
...
def hubspot_delete_meeting(access_token, params):
    ...
    try:
        if 'id' in params:
            ...
            requests.delete(url, headers=headers)
            return {"message":f"meeting of id {params['id']} successfully deleted"}
        else:
            raise Exception("missing input data")
    except Exception as e:
        raise Exception(e)
    ...
    try:
        if 'record' in params:
            ...
            response = requests.put(
                url=url, json=params['record'], headers=headers)
            result = response.json()
            if response.status_code == 200:
                return result
            else:
