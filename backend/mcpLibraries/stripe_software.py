...
def stripe_create_paymentintent(cred, params):
    ...
    try:
        ...
        if all(param in params for param in required_params) and "apiKey" in creds:
            api_key = creds["apiKey"]
            address_keys = ['line1', 'city', 'state', 'postal_code', 'country']
            address = {key: params.pop(key) for key in address_keys if params.get(key)}
            shipping={}
            if address:
                shipping["address"] = address
            for key in ["name", "phone", "tracking_number"]:
    ...
    try:
        ...
        if all(param in params for param in required_params) and "apiKey" in creds:
            ...
            if "shipping" in params and isinstance(params["shipping"], dict):
                ...
                address_keys = ['line1', 'city', 'state', 'postal_code', 'country']
                shipping_address = {key: shipping_data.pop(key) for key in address_keys if shipping_data.get(key)}
                shipping_info = {}
                if shipping_address:
...
def stripe_create_source(cred, params):
    ...
    try:
        ...
        if all(param in params for param in required_params) and "apiKey" in creds:
            ...
            result = json.loads(json.dumps(source, default=str))
            return result
        else:
            raise Exception("Missing input data")
    except Exception as e:
...
def stripe_create_coupon(cred, params):
    ...
    :param dict params: contains properties to be added to the created coupon
        ...
        - :currency: (str) Three-letter ISO currency code, in lowercase
        - :currency: (name) Name of the coupon displayed to customers on, for instance invoices, or receipts.
