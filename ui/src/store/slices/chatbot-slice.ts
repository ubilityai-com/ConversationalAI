...
import { ConstantVariable, DialogueVariables, OutputVariables } from "./variables-slice"
import chatbotApis from "../../api/chatbotApis"
import { createFlowObject } from "../../lib/build-json"
import { Node } from "./flow-slice"
...
export interface ChatbotSlice {
  ...
  saveBot: (data: Omit<BotSchema, "token" | "id" | "updated_date">) => Promise<BotSchema>
...
export const createChatbotSlice: StateCreator<any, [], [], ChatbotSlice> = (set, get) => ({
  selectedBot: null,
  botsList: [],
  isLoadingBot: false,
  ...
  updateSelectedBot: (partial) =>
    set((state: any) => ({
      selectedBot: state.selectedBot ? { ...state.selectedBot, ...partial } : state.selectedBot,
    })),
  ...
  fetchBots: async () => {
    ...
    try {
      ...
      set({
        error: error.response?.data?.message || error.message || "Failed to fetch bots",
        isLoadingBotList: false,
      })
    }
  },
  ...
  updateBot: async (data) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || "Failed to update bot"
      set({
        error: errorMessage,
        isLoadingBot: false,
      })
  ...
  loadBot: async (data) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || "Failed to load bot"
      set({
        error: errorMessage,
        isLoadingBot: false,
      })
  ...
  activateBot: async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || "Failed to activate bot"
      set({
        error: errorMessage,
        isLoadingActivate: false,
      })
