...
export function getContent(selectedNode: any, params: any) {
    ...
    const handlerCases = {
        Other: getNextNodeId(selectedNode.id, edges, nodes, "condition-agent-default"),
...
export default function ConditionAgentForm({
        ...
        useDebounceConfig<LLMConfigProps["rightSideData"]>(
            selectedNode.data.rightSideData,
            {
                ...
                onSave: (savedConfig) => {
                    ...
                    updateNodesValidationById(selectedNode.id, checkIfAllRequiredDataIsFilled(savedConfig) && subsValid)
                    console.log({ selectedNode, aa: checkIfAllRequiredDataIsFilled(savedConfig) && subsValid, subNodesValidation });
    ...
    const updateScenario = (scenarioId: string, updates: Partial<Scenario>) => {
        const updatedScenarios = scenarios.map((c) => (c.id === scenarioId ? { ...c, ...updates } : c))
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader className="pb-3">
                    ...
                    <CardDescription>Add options users can select. Every Scenario gets its own output handle.</CardDescription>
                </CardHeader>

                <CardContent className="space-y-4">
                    {scenarios.length === 0 && <p className="text-xs text-gray-500">No scenarios yet. Click “Add Scenario.</p>}

                    {scenarios.map((scenario, idx) => (
                        <Card key={scenario.id} className="border border-gray-200">
                            <CardContent className="p-4 space-y-3">
                                <div className="flex items-center justify-between">
                                    <span className="font-medium text-sm">Scenario {idx + 1}</span>
                                    <Button
                                        variant="ghost"
                                        size="sm"
                    ...
                    <Button variant="outline" size="sm" onClick={addScenario} className="h-8 w-full">
