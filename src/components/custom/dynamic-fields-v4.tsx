...
const DynamicInputFields: React.FC<DynamicInputFieldsProps> = (props) => {
    ...
    const updateFieldsArray = (newValue: any) => {
        if (onFieldChange && json.variableName) {
            ...
            onFieldChange({ path: json.variableName, value: newValue })
        }
    }
    ...
    const onRemoveVariables = (index: number) => {
        const newVariables = currentFieldsArray.filter((f: any, i: number) => i !== index)
            ...
            <div className={cn("rounded-l-md")}>
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {filledArray.map((fieldSet: Record<string, any>, fieldSetInd: number) => (
                            <Card key={fieldSetInd} className="border border-gray-200">
                                <CardContent className="p-4 space-y-3">
                                    {json.type !== "accordion" && <div className="flex items-center justify-between">
                        ...
                        {json.type !== "accordion" && <Button variant="outline" size="sm" onClick={onAddVariables} className="h-8 w-full bg-transparent">
