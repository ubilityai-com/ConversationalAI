...
function MicrosoftSignIn() {
    ...
    if (domain) {
        ...
        else scopes = [...scopesWithoutCustomUses];
    }
    const REDIRECT_URI = `${process.env.REACT_APP_DOMAIN}/dashboard/credentials`;
...
function LinkedInSignIn() {
    ...
    const REDIRECT_URI = `${process.env.REACT_APP_DOMAIN}/dashboard/credentials`;
    // const REDIRECT_URI = `http://localhost:3000/dashboard/credentials`;
    const scopesWithoutOrganization = [
        "w_member_social",
        "profile",
        "email",
        "openid",
...
function XSignIn() {
    ...
    const XUrl = `https://twitter.com/i/oauth2/authorize?client_id=${client_id}&redirect_uri=${encoded_redirect_uri}&scope=${scopes.join(" ")}&response_type=code&state=${state}&code_challenge=${code_challenge}&code_challenge_method=plain`;
    window.location.href = XUrl;
}
function ServiceNowSignIn() {
    function generateRandomString(length = 32) {
        const characters =
            "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
        let result = "";
    ...
    const ServiceNowUrl = `https://${Cookies.get("instanceDomain")}.service-now.com/oauth_auth.do?client_id=${client_id}&redirect_uri=${encoded_redirect_uri}&response_type=code&state=${state}`;
...
function RedditSignIn() {
    ...
    const encoded_redirect_uri = encodeURIComponent(REDIRECT_URI);
    const state = generateRandomString(15);
    const RedditUrl = `https://www.reddit.com/api/v1/authorize?client_id=${client_id}&response_type=code&state=${state}&redirect_uri=${encoded_redirect_uri}&scope=${scopesString}&duration=permanent`;
    window.location.href = RedditUrl;
}
function ZendeskSignIn() {
    const client_id = Cookies.get("clientId");
    const baseUrl = Cookies.get("baseUrl");
    const scopes = ["read", "write"];
    const REDIRECT_URI = `${process.env.REACT_APP_DOMAIN}/dashboard/credentials`;
    const encoded_redirect_uri = encodeURIComponent(REDIRECT_URI);
    const ZendeskUrl = `https://${baseUrl}.zendesk.com/oauth/authorizations/new?response_type=code&redirect_uri=${encoded_redirect_uri}&client_id=${client_id}&scope=${scopes.join(" ")}`;
    window.location.href = ZendeskUrl;
}
function ClickUpSignIn() {
    const client_id = Cookies.get("clientID");
    const REDIRECT_URI = `${process.env.REACT_APP_DOMAIN}/dashboard/credentials`;
    // const REDIRECT_URI = `http://localhost:3000/dashboard/credentials`;
    const encoded_redirect_uri = encodeURIComponent(REDIRECT_URI);
    const ClickUpUrl = `https://app.clickup.com/api?response_type=code&redirect_uri=${encoded_redirect_uri}&client_id=${client_id}`;
    window.location.href = ClickUpUrl;
}
function MailChimpSignIn() {
    const client_id = Cookies.get("clientID");
    const REDIRECT_URI = `${process.env.REACT_APP_DOMAIN}/dashboard/credentials`;
    const encoded_redirect_uri = encodeURIComponent(REDIRECT_URI);
    const MailChimpUrl = `https://login.mailchimp.com/oauth2/authorize?response_type=code&redirect_uri=${encoded_redirect_uri}&client_id=${client_id}`;
    window.location.href = MailChimpUrl;
}
function SnowflakeSignIn() {
    const client_id = Cookies.get("clientId");
    const accountId = Cookies.get("accountId");
    const REDIRECT_URI = `${process.env.REACT_APP_DOMAIN}/dashboard/credentials`;
    const encoded_redirect_uri = encodeURIComponent(REDIRECT_URI);
    const snowFlakeUrl = `https://${accountId}.snowflakecomputing.com/oauth/authorize?response_type=code&client_id=${client_id}&redirect_uri=${encoded_redirect_uri}&scope=refresh_token`;
    window.location.href = snowFlakeUrl;
}
function SlackSignIn() {
    ...
    // const REDIRECT_URI = `http://localhost:3000/dashboard/credentials`;
    const encoded_redirect_uri = encodeURIComponent(REDIRECT_URI);
    const SlackUrl = `https://slack.com/oauth/v2/authorize?response_type=code&redirect_uri=${encoded_redirect_uri}&client_id=${client_id}&scope=${scopes.join(",")}&user_scope=${userScopes.join(",")}`;
