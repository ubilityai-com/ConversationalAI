...
export function ConditionAgentNode(props: NodeProps<Node<FormNodeData>>) {
  ...
  const handles = (
    <>
      {(data.rightSideData.scenarios || []).map(
      ...
      <div className="mt-4 space-y-2">
        {data.rightSideData.scenarios.map(
          (scenario: Scenario, index: number) => (
            <div
              key={index}
              className="flex items-center text-xs bg-yellow-50 rounded-lg p-3"
            >
              <div className="w-2 h-2 bg-yellow-500 rounded-full mr-2"></div>
              <span className="font-medium text-yellow-700 mr-2 truncate">
                {scenario.label}
              </span>
            </div>
          )
