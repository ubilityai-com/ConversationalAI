...
export function checkIfAllNodesConnected(
  nodes: NodeType[],
  edges: EdgeType[]
): boolean {
  ...
  for (const node of nodes) {
    ...
    if (node.type === "Handler") {
      ...
      if (outgoing.length === 0) {
        console.warn(
          `Node ${node.id} of type Handler has no outgoing connections.`
        );
        return false;
      }
      ...
      if (incoming.length === 0) {
        console.warn(
          `Node ${node.id} of type End has no incoming connections.`
        );
        return false;
      }
      ...
      if (incoming.length === 0) {
        console.warn(
          `Node ${node.id} of type ChoicePrompt has no incoming connections.`
        );
        return false;
      }
      ...
      if (incoming.length === 0) {
        console.warn(
          `Node ${node.id} of type ${node.type} has no incoming connections.`
        );
        return false;
      }
      if (outgoing.length === 0) {
        console.warn(
          `Node ${node.id} of type ${node.type} has no outgoing connections.`
        );
        return false;
      }
    }
